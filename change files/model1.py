...
class TATransEModel(nn.Module):
    ...
    def forward(self,loss_type, pos_h, pos_t, pos_r, pos_tem, neg_h, neg_t, neg_r, neg_tem):
        print(loss_type)
        # exit()
        pos_h_e = self.ent_embeddings(pos_h)
        pos_t_e = self.ent_embeddings(pos_t)
        ...
        if loss_type == 0:
            ...
            pos_t = pos_h_e + pos_rseq_e
            neg_t = neg_h_e + neg_rseq_e
            pos_h = pos_t_e - pos_rseq_e
            neg_h =  neg_t_e - neg_rseq_e
            # print(type(self.ent_embeddings))
            # exit()
            if self.L1_flag :
                print("data")
                print(type(self.ent_embeddings))
                dist1 = pairwise_distances(pos_h , self.ent_embeddings, metric ='manhattan')
                dist2 = pairwise_distances(pos_t , self.ent_embeddings, metric ='manhattan')
                # print(dist1.shape)
                exit()
